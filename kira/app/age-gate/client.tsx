    ...
    <>
      <header className="flex justify-between items-center px-4 pt-4">
        <button onClick={() => router.back()} aria-label="Back">
          <Icon name="chevron-left" size={22} className="text-text-dim" />
        </button>
        <KiraLogo size={14} />
        <div className="w-6" />
      </header>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 max-w-md mx-auto w-full"
          ...
          {[
            ...
            <li key={i}>
              <button type="button"
                ...
                {/* FIX: replaced invalid w-5.5/h-5.5 Tailwind classes with inline style */}
                <span
                  className={`rounded-md grid place-items-center flex-shrink-0 border${
        ...
        <button onClick={() => router.push('/')}
          className="kira-btn bg-transparent border-none text-text-muted mt-2 hover:bg-transparent">
          Not now
        </button>
