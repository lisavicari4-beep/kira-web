// app/api/search/route.ts — AI Smart Search (SSE stream).
...
async function makePlan(q: string): Promise<Plan> {
  ...
  // FIX: correct versioned model string (was "claude-haiku-4-5" → 404 from API)
  ...
  catch { return { keywords: [q], filters: {}, tone: 'neutral' }; }
}
...
function rankHits(titles: Title[], plan: Plan, q: string) {
  ...
  // FIX: normalise match % so low-relevance results don't show 65%+ by default
  return scored.slice(0, 6).map(({ t, score }, i) => ({
    rank: i + 1,
    title: t,
    match: Math.max(30, Math.min(99, Math.round((score / topScore) * 99))),
