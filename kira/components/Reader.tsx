'use client';
// components/Reader.tsx — three orientations, client-side interaction.
...
interface Props {
  chapter: Chapter; titleId: string; titleName: string; initialMode?: Mode;
}
...
export function Reader({ chapter, titleId, titleName, initialMode = 'vertical' }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0a0a0c] flex flex-col">
      ...
      <BottomChrome chapter={chapter} page={page} mode={mode} setMode={setMode}
        readAlong={readAlong} setReadAlong={setReadAlong} hidden={chromeHidden} />
    </div>
  );
}

function TopChrome({ titleId, titleName, chapter, hidden }: {
  titleId: string; titleName: string; chapter: Chapter; hidden: boolean;
}) {
  return (
    <div className={clsx('absolute top-0 left-0 right-0 z-20 px-4 py-3 transition-transform', hidden && '-translate-y-full')}
...
function Vertical({ chapter, readAlong, onPage }: {
  chapter: Chapter; readAlong: boolean; onPage: (n: number) => void;
}) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-y-auto pt-20 pb-32"
      onClick={(e) => e.stopPropagation()}>
      <div className="max-w-md mx-auto flex flex-col gap-2 px-4">
        {chapter.pages.map((p) => <PageView key={p.idx} page={p} readAlong={readAlong} />)}
        <EndOfChapter />
      </div>
    </div>
  );
...
function Horizontal({ chapter, page, setPage, mode, readAlong }: {
  ...
  return (
    <div className="absolute inset-0 pt-20 pb-32 flex items-center justify-center px-4"
      onClick={(e) => e.stopPropagation()}>
...
function PageView({ page, readAlong }: { page: ChapterPage; readAlong: boolean }) {
  return (
    <div className="relative bg-paper rounded shadow-lg overflow-hidden">
      <img src={page.imageUrl} alt={`Page ${page.idx + 1}`}
        loading="lazy" decoding="async" className="block w-full h-auto select-none" />
...
function BottomChrome({ chapter, page, mode, setMode, readAlong, setReadAlong, hidden }: {
  ...
  return (
    <div className={clsx('absolute bottom-0 left-0 right-0 z-20 pb-7 transition-transform', hidden && 'translate-y-full')}
