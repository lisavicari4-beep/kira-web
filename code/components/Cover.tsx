// Cover — placeholder cover with neon-shōnen graphic when no image,
// or the real image with consistent radius + halftone overlay.
...
export interface CoverProps {
  ...
  className?: string;
  priority?: boolean;
  /** When true, halftone + speedlines drawn on top of image */
  showTexture?: boolean;
}
...
export function Cover({ src, alt, paletteSeed = 0, ratio = 'portrait', className = '', priority, showTexture = false }: CoverProps) {
  ...
  // No image: render a placeholder
  return (
    <div
      className={`relative overflow-hidden rounded-md ${aspect} ${className}`}
      style={{
        background: `radial-gradient(circle at 30% 25%, ${acc}66, transparent 55%), linear-gradient(160deg, ${bg1}, ${bg2})`,
      }}
      aria-label={alt}
      role="img"
    >
