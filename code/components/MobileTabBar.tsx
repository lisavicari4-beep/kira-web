// MobileTabBar — only renders on < md (DesktopNav takes over above).
'use client';
...
export function MobileTabBar() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 pb-7 pt-2.5"
      ...
      <ul className="flex justify-around items-end">
        {TABS.map((t) => {
          ...
          if (t.icon === 'plus') return (
            <li key={t.id}>
              <Link href={t.href} className="bg-hot rounded-2xl w-12 h-12 -translate-y-3 grid place-items-center shadow-hot-soft" aria-label="Create">
          ...
          return (
            <li key={t.id}>
              <Link href={t.href}
                className={clsx('flex flex-col items-center gap-0.5 px-2',
