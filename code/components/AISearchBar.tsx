// AISearchBar — chat-style smart search; streams results from /api/search.
'use client';
...
import type { SearchHit, Title } from '@/lib/types';
  ...
  return (
    <div className="flex flex-col gap-4">
      <form onSubmit={submit} className="flex items-center gap-2">
        ...
        <button type="submit" disabled={busy || !q.trim()} className="w-11 h-11 grid place-items-center rounded-full bg-hot text-white shadow-hot-soft disabled:opacity-50">
...
function HitCard({ h }: { h: SearchHit }) {
  return (
    <li>
      <Link href={`/title/${h.title.id}`} className="flex gap-3 p-2.5 bg-bg-1 rounded-xl border border-line hover:border-line-strong">
