...
export function sampleChapter(titleId: string): Chapter {
  const pages = Array.from({ length: 8 }).map((_, i) => ({
    ...
    height: 1200,
    jpTokens: i === 0 ? [
      { text: '門', reading: 'mon', meaning: 'gate', pos: 'noun', jlpt: 'N4' as const, box: [$1, $2, $3, $4] as [number, number, number, number] },
      { text: '息', reading: 'iki', meaning: 'breath', pos: 'noun', jlpt: 'N3' as const, box: [$1, $2, $3, $4] as [number, number, number, number] },
    ] : [],
  }));
  return {
    id: `${titleId}-ch-1`,
